...
def get_mw_mean_med_vrc(data, ws):
    ...
    for i in range(data.shape[0] - ws):
        ...
        min_arr[i] = data[i:i + ws].min()
        max_arr[i] = data[i:i + ws].max()
    ...
    probs = np.arange(1.0, mean_arr.size + 1.0) / (mean_arr.size + 1.0)

    return (probs, mean_arr, med_arr, vrc_arr, min_arr, max_arr)
...
def main():
    ...
    out_dir = Path(sim_dir / 'mw_stat_figs_dist')
    ...
    for ws in windows_sizes:
        for i in range(n_cols):
            ...
            x_crds, mean_arr, med_arr, vrc_arr, min_arr, max_arr = get_mw_mean_med_vrc(
                ref_data[:, i], ws)
            ...
            plt.plot(mean_arr, x_crds, label=label, alpha=0.7, lw=2, c='red')

            plt.figure(meds_fig.number)
            plt.plot(med_arr, x_crds, label=label, alpha=0.7, lw=2, c='red')

            plt.figure(vrcs_fig.number)
            plt.plot(vrc_arr, x_crds, label=label, alpha=0.7, lw=2, c='red')

            plt.figure(mins_fig.number)
            plt.plot(min_arr, x_crds, label=label, alpha=0.7, lw=2, c='red')

            plt.figure(maxs_fig.number)
            plt.plot(max_arr, x_crds, label=label, alpha=0.7, lw=2, c='red')
            ...
            for j in range(n_sims):
                ...
                plt.figure(means_fig.number)
                plt.plot(mean_arr, x_crds, label=label, alpha=0.3, lw=1, c='gray')

                plt.figure(meds_fig.number)
                plt.plot(med_arr, x_crds, label=label, alpha=0.3, lw=1, c='gray')

                plt.figure(vrcs_fig.number)
                plt.plot(vrc_arr, x_crds, label=label, alpha=0.3, lw=1, c='gray')

                plt.figure(mins_fig.number)
                plt.plot(min_arr, x_crds, label=label, alpha=0.3, lw=1, c='gray')

                plt.figure(maxs_fig.number)
                plt.plot(max_arr, x_crds, label=label, alpha=0.3, lw=1, c='gray')

            plt.figure(means_fig.number)
            plt.ylabel('Probability')
            plt.xlabel(f'Moving window mean ({ws} steps)')
            plt.grid()
            plt.legend()
            ...
            plt.figure(meds_fig.number)
            plt.ylabel('Probability')
            plt.xlabel(f'Moving window median ({ws} steps)')
            plt.grid()
            plt.legend()
            ...
            plt.figure(vrcs_fig.number)
            plt.ylabel('Probability')
            plt.xlabel(f'Moving window variance ({ws} steps)')
            plt.grid()
            plt.legend()
            ...
            plt.figure(mins_fig.number)
            plt.ylabel('Probability')
            plt.xlabel(f'Moving window minima ({ws} steps)')
            plt.grid()
            plt.legend()
            ...
            plt.figure(maxs_fig.number)
            plt.ylabel('Probability')
            plt.xlabel(f'Moving window maxima ({ws} steps)')
            plt.grid()
            plt.legend()
